    ...
    for line in text.splitlines():
        ...
        if nchars > 0:
            ...
            if f_upper >= 0.5:  # Mostly uppercase characters
                ...
                split_on_punctuation = punctuation_ex.split(
                    line.replace('l', 'I'))
                line = ''.join([sentence.capitalize()
                               for sentence in split_on_punctuation])
...
def clean_nowandme(text, seq_length):
    pad_seq = _make_padding_sequence(seq_length)
    text = cleanup_extra_spaces(text)
    text = START_TOKEN + re.sub(r'\n\n', pad_seq + START_TOKEN, text) + pad_seq
    return text
